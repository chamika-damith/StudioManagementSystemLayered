...
public class OrderItemDetailFormModel {
    public List<OrderItemDetailFormDto> getAllValues(int id) throws SQLException {
        ...
        String sql="SELECT orders.orderId,orders.price,item.itemId,od.qty,item.description,item.price,item.name,item.category,item.img FROM orders JOIN order_detail od on orders.orderId = od.orderId JOIN item item on od.itemId = item.itemId WHERE od.orderId=?";
