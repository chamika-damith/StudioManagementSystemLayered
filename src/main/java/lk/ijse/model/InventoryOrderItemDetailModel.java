...
public class InventoryOrderItemDetailModel {
    public List<InventoryOrderItemDto> getAllValues(int id) throws SQLException {
        ...
        String sql="SELECT sod.itemId,sod.qty,so.description,so.category,item.name,item.price FROM supplier_order so JOIN suporderdetail sod on so.supOrderId = sod.supOrderId JOIN item item on sod.itemId = item.itemId WHERE sod.supOrderId=?";
